...
*official* binaries: hm2mqtt solved the runtime problem by repackaging
...
1. The whole load chain hangs off a single jQuery `change` handler on the
   ...
   `oneditprepare` (three nodes already do this trick for the *iface*
   select, none for the *config* select).
